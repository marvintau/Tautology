...
Tautology.Canvas.prototype.makePatterns = function(){
	...
	for(var i = 0; i < Math.floor($('.canvas-container').width()/50); i++){
		for(var j = 0; j < Math.floor($('.canvas-container').height()/50); j++){
	...
	var stripesList = [];
	for(var i = 0; i < Math.floor($('.canvas-container').width()/40) + 50; i++){

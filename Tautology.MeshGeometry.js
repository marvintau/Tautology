Tautology.MeshGeometry = function(){
	this.left;
	this.top;
	this.width;
	this.height;
...
Tautology.MeshGeometry.prototype = {
	...
	init : function(array, left, width, top, height){
		this.left = left;
		this.top = top;
		this.width = width;
		this.height = height;
	...
	updateQuad : function(i, j){
		this.updateFace(this.vertexTable[i+","+j],
						this.vertexTable[(i+1)+","+j],
						this.vertexTable[i+","+(j+1)]);
		this.updateFace(this.vertexTable[i+","+(j+1)], 
						this.vertexTable[(i+1)+","+j],
						this.vertexTable[(i+1)+","+(j+1)]);
	},
	...
	updateGeom : function(swapAxis){
		...
		if(swapAxis){
			this.geom.faceVertexUvs[0].map(function(face){
				face.map(function(point){point.set(point.y, point.x);});
			});
		}
	...
	generateGeom : function(swapAxis){
		...
		// console.log(this.geom.faceVertexUvs[0]);
